...
def get_range(col, df1, df2):
    ...
    '''
    minValue = df1[col].min() if df1[col].min() < df2[col].min() else df2[col].min()
    maxValue = df1[col].max() if df1[col].min() > df2[col].max() else df2[col].max()
...
def sort_dy_by_yr_continent(my_df):
    ...
    '''
    return my_df.sort_values(['Year', 'Continent'], ascending=[True, True])
